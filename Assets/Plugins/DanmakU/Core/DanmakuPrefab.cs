...
namespace DanmakU
{
    ...
    public sealed class DanmakuPrefab : BetterBehaviour
    {
        ...
        [SerializeField] private ParticleSystem danmakuSystemPrefab;

        [Serialize, Default(Enum = 0)] internal Danmaku.ColliderType collisionType;

        [Serialize, Default(1f, 1f)] internal Vector2 colliderSize;

        [Serialize, Default(0f, 0f)] internal Vector2 colliderOffset;
        ...
        public override void Reset()
        {
            base.Reset();
            danmakuSystemPrefab =
                (Resources.Load("Danmaku Particle System") as GameObject).GetComponent<ParticleSystem>();
        ...
        [Serialize] internal bool fixedAngle;

        [Serialize] private IDanmakuController[] extraControllers;
        ...
        private void OnDrawGizmosSelected()
        {
            ...
            Matrix4x4 hitboxMatrix = Matrix4x4.TRS((Vector2) transform.position,
                Quaternion.Euler(0f, 0f, transform.eulerAngles.z), transform.lossyScale);
            ...
            switch (collisionType)
            {
                ...
                case Danmaku.ColliderType.Circle:
                    hitboxMatrix = Matrix4x4.TRS((Vector2) transform.position,
                        Quaternion.Euler(0f, 0f, transform.Rotation2D()), transform.lossyScale.Max()*Vector3.one);
                    Gizmos.matrix = hitboxMatrix;
                    Handles.matrix = hitboxMatrix;
