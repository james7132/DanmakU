...
public class PolySpeedDanmaku : MonoBehaviour, IDanmakuModifier {
  ...
  struct PolySpeed : IJob, IJobParallelFor {
    ...
    public void Execute(int index) {
		...
		Speeds[index] = 0;
		for(int i=0;i < Coefficients.Length;i++){
			if(Coefficients[i] != 0 && !((System.Math.Abs(elapsedTime) < 0.0001f || elapsedTime > 5f) && Powers[i] < 0)){ // prevent divide giant numbers or super small numbers
				Speeds[index] += Coefficients[i] * (float)System.Math.Pow(elapsedTime,Powers[i]);
			}
		}
    }
